        ...
        for (ResourceLocation name : TILE_REGISTRY.getKeys())
        {
            ...
            if (name != null && type != null)
            {
                try
                {
                    ...
                    boolean shouldBan = HandlerManager.INSTANCE.tileEntityBanList.contains(type) || typeString.contains("cable") || typeString.contains("wire") || typeString.contains("pipe") || typeString.contains("tube") || typeString.contains("conduit") || typeString.contains("channel");
            ...
            for (ResourceLocation name : TILE_REGISTRY.getKeys())
            {
                ...
                if (name != null && type != null)
                {
                    try
                    {
                        ...
                        else if (shouldBan && HandlerManager.INSTANCE.tileEntityBanList.contains(type))
                        {
                            //If original was banned but someone unbanned it in the config
                            HandlerManager.INSTANCE.tileEntityBanList.remove(type);
                        }
                    }
                    catch (Exception e)
                    {
                        LOGGER.error("ModHandler#loadHandlerData() -> Failed to add entry to config [" + name + " > " + type.toString() + "]", e);
                    }
                }
            }
        }
